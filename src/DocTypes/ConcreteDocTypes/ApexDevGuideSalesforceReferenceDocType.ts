...
export class ApexDevGuideSalesforceReferenceDocType extends SalesforceReferenceAtlasDocType {
    constructor() {
        super(
            DocTypeName.APEX_DEV_GUIDE,
            'apexcode'
        );
    }
    /**
    ...
    protected async getRootDocumentationNode(): Promise<SalesforceAtlasTOC.DocumentationNode> {
        ...
        return apexDocToc.toc.find((node: SalesforceAtlasTOC.DocumentationNode) => node.hasOwnProperty('id') && node.id === 'apex_dev_guide');
    }
}