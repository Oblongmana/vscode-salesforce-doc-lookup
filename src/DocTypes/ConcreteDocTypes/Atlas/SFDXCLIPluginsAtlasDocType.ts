...
export class SFDXCLIPluginsAtlasDocType extends AtlasDocType {
    constructor() {
        super(
            DocTypeID.SFDX_CLI_PLUGINS
        );
    }
    /**
    ...
    protected async getRootDocumentationNode(): Promise<AtlasTOC.DocumentationNode> {
        ...
        return docToc.toc.find((node: AtlasTOC.DocumentationNode) => node.hasOwnProperty('id') && node.id === 'cli_plugins');
    }
}