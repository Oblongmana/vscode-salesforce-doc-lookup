...
export class ConnectChatterAPIAtlasDocType extends AtlasDocType {
    constructor() {
        super(
            DocTypeID.CONNECT_CHATTER_API
        );
    }
    ...
    protected async getRootDocumentationNode(): Promise<AtlasTOC.DocumentationNode> {
        ...
        return docToc.toc.find((node: AtlasTOC.DocumentationNode) => node.hasOwnProperty('id') && node.id === 'intro_what_is_chatter_connect');
    }
}